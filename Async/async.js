const usuarios = [
    {nome: 'Leonardo', sobrenome: 'Martins', id: 5},
    {nome: 'Mário', sobrenome: 'Martins', id: 3},
    {nome: 'Andreia', sobrenome: 'Martins', id: 1},
    {nome: 'Maya', sobrenome: 'Martins', id: 6},
]

function getUsers() {
    let resultado = '';
    usuarios.forEach(usuarios => resultado += `<li> ${usuarios.nome} ${usuarios.sobrenome} ${usuarios.id}`);
    document.body.innerHTML = resultado; 
}

async function promiseUsers() {
    const busca = await fetch('https://jsonplaceholder.typicode.com/users');
    const json2 = await busca.json(); 
    json2.forEach(json => document.body.innerHTML = (JSON.stringify(`id: ${json.id} <br> nome: ${json.name}`)));
    
    


}


 function createUsers(users) {
    return new Promise((resolve, reject) => {
        usuarios.push(users);

        const error = false;

        if(!error) {
            resolve();
        } else {
            reject('Erro');
        }
    })
}

getUsers();

createUsers({nome: 'Iracema', sobrenome: 'Fernandes', id: 9})
.then(getUsers);



